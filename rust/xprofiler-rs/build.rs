...
fn main() {
    ...
    match target_arch.as_str() {
        ...
        _ => {
            println!("cargo:warning=Unsupported target architecture: {}", target_arch);
        }
    }
...
"#,
        version, build_time, target, profile, commit_hash, branch,
        version, branch, commit_hash, build_time, target, profile
    );
    
    std::fs::write(&dest_path, build_info_content)
        .expect("Failed to write build info module");
