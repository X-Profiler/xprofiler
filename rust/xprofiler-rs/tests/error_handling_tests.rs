...
mod error_handling_tests {
  ...
  fn test_libuv_monitor_invalid_handle_operations() {
    ...
    // Try to unregister non-existent handle
    let _ = libuv_monitor.unregister_handle("999999");

    // These operations should not panic or cause errors
    let stats = libuv_monitor
      .get_stats_for_period(TimePeriod::TenSeconds)
      .unwrap();
  ...
  fn test_monitor_reset_during_operation() {
    ...
    let _handle1 = libuv_monitor
      .register_handle("handle1".to_string(), HandleType::Tcp)
      .unwrap();
    let _handle2 = libuv_monitor
      .register_handle("handle2".to_string(), HandleType::Tcp)
      .unwrap();
    ...
    // Create measurement data to have stats to check
    use std::collections::HashMap;
    let mut handles_by_type = HashMap::new();
    handles_by_type.insert(HandleType::Tcp, 2);
    let phase_times = HashMap::new();
    libuv_monitor
      .record_measurement(1.0, 2, handles_by_type, 0, 10, phase_times)
      .unwrap();

    let stats_before = libuv_monitor
      .get_stats_for_period(TimePeriod::TenSeconds)
      .unwrap();
    assert_eq!(stats_before.total_handles, 2);
    assert_eq!(stats_before.loop_metrics.iterations, 10);

    // Reset during operation
    libuv_monitor.reset().unwrap();

    let stats_after = libuv_monitor
      .get_stats_for_period(TimePeriod::TenSeconds)
      .unwrap();
    assert_eq!(stats_after.total_handles, 0);
    assert_eq!(stats_after.loop_metrics.iterations, 0);
  ...
  fn test_high_frequency_http_requests() {
    ...
    while start_time.elapsed() < Duration::from_secs(1) {
      ...
      let response = HttpResponse {
        ...
        headers_size: 256,
        body_size: 128,
        response_time: Duration::from_millis(1), // Fast but measurable response
      };
