...
mod memory_overhead_tests {
    ...
    fn test_cpu_monitor_memory_overhead() {
        ...
        // CPU monitor should use less than 2MB of additional memory
        assert!(memory_overhead < 2 * 1024 * 1024, 
                "CPU monitor memory overhead too high: {} bytes", memory_overhead);
    }
    ...
    fn test_memory_monitor_memory_overhead() {
        ...
        // Memory monitor should use less than 2MB of additional memory
        assert!(memory_overhead < 2 * 1024 * 1024, 
                "Memory monitor memory overhead too high: {} bytes", memory_overhead);
    }
    ...
    fn test_gc_monitor_memory_overhead() {
        ...
        // GC monitor should use less than 2MB even with 1000 events
        assert!(memory_overhead < 2 * 1024 * 1024, 
                "GC monitor memory overhead too high: {} bytes", memory_overhead);
    }
    ...
    fn test_http_monitor_memory_overhead() {
        ...
        assert!(memory_overhead < 5 * 1024 * 1024, 
                "HTTP monitor memory overhead too high: {} bytes", memory_overhead);
    }
    ...
    fn test_libuv_monitor_memory_overhead() {
        let (memory_before, memory_after) = measure_memory_usage(|| {
            ...
            for i in 0..100 {
                ...
                let handle = monitor.register_handle(handle_type, true, i % 2 == 0);
                handles.push(handle);
            }
            ...
            for handle in handles {
                monitor.unregister_handle(handle);
            }
        ...
        // Libuv monitor should use less than 2MB
        assert!(memory_overhead < 2 * 1024 * 1024, 
                "Libuv monitor memory overhead too high: {} bytes", memory_overhead);
    }
    ...
    fn test_all_monitors_combined_memory_overhead() {
        ...
        let memory_overhead = memory_after.saturating_sub(memory_before);
        println!("Combined monitors memory overhead: {} bytes", memory_overhead);
        
        // All monitors combined should use less than 10MB
        assert!(memory_overhead < 10 * 1024 * 1024, 
                "Combined monitors memory overhead too high: {} bytes", memory_overhead);
    }
}
...
mod memory_leak_tests {
    ...
    fn test_monitor_lifecycle_no_memory_leak() {
        ...
        for cycle in 0..10 {
            ...
            if cycle % 3 == 0 {
                ...
                println!("Memory growth after {} cycles: {} bytes", cycle + 1, memory_growth);
    ...
    fn test_long_running_monitor_memory_stability() {
        ...
        if memory_samples.len() >= 3 {
            ...
            let late_avg = memory_samples[memory_samples.len()-2..].iter().sum::<usize>() / 2;
            ...
            assert!(growth_trend < 3 * 1024 * 1024, 
                    "Memory growth trend too high: {} bytes", growth_trend);
        }
        
        println!("Total memory growth in long run: {} bytes", total_growth);
        assert!(total_growth < 5 * 1024 * 1024, 
                "Long-running memory growth too high: {} bytes", total_growth);
    }
}