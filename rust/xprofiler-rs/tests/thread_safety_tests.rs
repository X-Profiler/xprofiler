...
mod concurrent_access_tests {
        ...
        thread::spawn(move || {
          ...
          for i in 0..iterations_per_thread {
            ...
            if let Ok(mut mon) = monitor.try_lock() {
              ...
              assert!(
                result.is_ok(),
                "Update failed in thread {} iteration {}: {:?}",
                thread_id,
                i,
                result.err()
              );
        ...
        thread::spawn(move || {
          ...
          for i in 0..iterations_per_thread {
            if let Ok(mut mon) = monitor.try_lock() {
              ...
              assert!(
                stats_result.is_ok(),
                "Get stats failed in thread {} iteration {}: {:?}",
                thread_id,
                i,
                stats_result.err()
              );
  ...
  fn test_http_monitor_concurrent_request_recording() {
    ...
    let mut monitor = monitor.lock().unwrap();
    let stats = monitor.get_stats().unwrap();
    let stats_period = stats.get(&TimePeriod::TenSeconds).unwrap();
    println!(
      "Total HTTP requests recorded: {}",
      stats_period.total_requests
    );
    assert!(stats_period.total_requests > 0);
    monitor.stop().unwrap();
  }
  ...
  fn test_libuv_monitor_concurrent_handle_operations() {
        ...
        thread::spawn(move || {
          ...
          for i in 0..operations_per_thread {
            if let Ok(mut mon) = monitor.try_lock() {
              ...
              let handle_id = format!("{}_{}_{}", thread_id, i, handle_type.as_str());
              mon.register_handle(handle_id.clone(), handle_type).unwrap();
              thread_handles.push(handle_id);

              // Record loop iteration
              mon.record_loop_iteration().unwrap();
            }
          ...
          // Unregister handles
          for handle_id in thread_handles {
            if let Ok(mut mon) = monitor.try_lock() {
              mon.unregister_handle(&handle_id).unwrap();
            }
          }
        })
      })
    ...
    let stats = monitor.get_stats().unwrap();
    let stats_period = stats.get(&TimePeriod::TenSeconds).unwrap();
    println!("Total libuv handles: {}", stats_period.total_handles);
    assert!(stats_period.total_handles >= 0);
    monitor.stop().unwrap();
  }
}
  ...
  fn test_monitor_state_consistency_under_load() {
    ...
    for thread_id in 0..num_consumer_threads {
      ...
      let handle = thread::spawn(move || {
        ...
        for i in 0..200 {
          if let Ok(mon) = monitor.lock() {
            if let Ok(stats) = mon.get_stats() {
              if let Some(stats_period) = stats.get(&TimePeriod::TenSeconds) {
                // Verify that request count is monotonically increasing
                assert!(
                  stats_period.total_requests >= last_request_count,
                ...
                last_request_count = stats_period.total_requests;

                // Verify basic consistency
                assert!(
                  stats_period.avg_response_time <= 1000.0,
                  "Average response time too high: {}",
                  stats_period.avg_response_time
                );
              }
            }
          }
        ...
        println!(
          "Consumer {} observed {} total requests",
          thread_id, last_request_count
        );
      });
    ...
    if let Some(stats_period) = final_stats.get(&TimePeriod::TenSeconds) {
      println!(
        "Expected requests: {}, Actual requests: {}",
        expected_requests, stats_period.total_requests
      );

      assert_eq!(
        stats_period.total_requests, expected_requests as u64,
