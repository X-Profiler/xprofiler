...
impl BenchmarkResult {
  fn new(operation: String, durations: Vec<Duration>) -> Self {
    ...
    let p95_latency = sorted_durations
      .get(p95_index.min(sorted_durations.len() - 1))
      .copied()
      .unwrap_or(Duration::ZERO);
    let p99_latency = sorted_durations
      .get(p99_index.min(sorted_durations.len() - 1))
      .copied()
      .unwrap_or(Duration::ZERO);
...
mod monitor_performance_tests {
  ...
  fn benchmark_cpu_monitor_update() {
    ...
    assert!(
      ...
      "CPU monitor update too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(10),
      "CPU monitor P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_memory_monitor_update() {
    ...
    assert!(
      ...
      "Memory monitor update too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(20),
      "Memory monitor P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_gc_monitor_event_recording() {
    ...
    assert!(
      ...
      "GC event recording too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(1),
      "GC event recording P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_http_monitor_request_recording() {
    ...
    assert!(
      ...
      "HTTP request recording too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(5),
      "HTTP request recording P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_libuv_monitor_loop_recording() {
    ...
    assert!(
      ...
      "Libuv loop recording too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_micros(500),
      "Libuv loop recording P99 latency too high: {:?}",
      result.p99_latency
    );
...
mod stats_retrieval_performance_tests {
  ...
  fn benchmark_cpu_stats_retrieval() {
    ...
    assert!(
      result.operations_per_second > 10000.0,
      "CPU stats retrieval too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(1),
      "CPU stats retrieval P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_memory_stats_retrieval() {
    ...
    assert!(
      result.operations_per_second > 500.0,
      "Memory stats retrieval too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(5),
      "Memory stats retrieval P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_gc_stats_retrieval() {
    ...
    assert!(
      result.operations_per_second > 2000.0,
      "GC stats retrieval too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(5),
      "GC stats retrieval P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_http_stats_retrieval() {
    ...
    assert!(
      result.operations_per_second > 2000.0,
      "HTTP stats retrieval too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(5),
      "HTTP stats retrieval P99 latency too high: {:?}",
      result.p99_latency
    );
  ...
  fn benchmark_libuv_stats_retrieval() {
    ...
    assert!(
      ...
      "Libuv stats retrieval too slow: {:.2} ops/sec",
      result.operations_per_second
    );
    assert!(
      result.p99_latency < Duration::from_millis(2),
      "Libuv stats retrieval P99 latency too high: {:?}",
      result.p99_latency
    );
...
mod concurrent_performance_tests {
  ...
  fn benchmark_concurrent_cpu_monitor_access() {
    ...
    assert!(
      ...
      "Concurrent CPU monitor access too slow: {:.2} ops/sec",
      ops_per_second
    );
  ...
  fn benchmark_mixed_monitor_workload() {
    ...
    assert!(
      ...
      "Mixed monitor workload too slow: {:.2} ops/sec",
      ops_per_second
    );
...
mod overhead_measurement_tests {
  ...
  fn measure_monitoring_overhead() {
    ...
    // Overhead should be less than 200% (monitoring can add significant overhead)
    assert!(
      overhead_percentage < 200.0,
      "CPU monitoring overhead too high: {:.2}%",
      overhead_percentage
    );
  }
  ...
  fn measure_all_monitors_overhead() {
    ...
    println!(
      "\nTotal Monitoring Overhead: {:.2}%",
      total_overhead_percentage
    );

    // Total overhead should be less than 2000% (monitoring can be expensive but should be reasonable)
    assert!(
      total_overhead_percentage < 2000.0,
      "Total monitoring overhead too high: {:.2}%",
      total_overhead_percentage
    );
  }
}
