...
mod http_tests {
  ...
  fn test_http_request_recording() {
    ...
    assert_eq!(stats_period.total_responses, 1);
    assert_eq!(stats_period.status_codes.get(&500), Some(&1));
...
mod libuv_tests {
  ...
  fn test_handle_registration() {
    ...
    let handle_id = "timer_1".to_string();
    monitor
      .register_handle(handle_id.clone(), HandleType::Timer)
      .unwrap();
  ...
  fn test_handle_unregistration() {
    ...
    let handle_id = "tcp_1".to_string();
    monitor
      .register_handle(handle_id.clone(), HandleType::Tcp)
      .unwrap();
    monitor.unregister_handle(&handle_id).unwrap();

    let stats = monitor.get_stats().unwrap();
    let stats_period = stats.get(&TimePeriod::TenSeconds).unwrap();
    assert_eq!(stats_period.total_handles, 0);
  ...
  fn test_loop_iteration_recording() {
    ...
    // Record loop iteration (increments counter)
    monitor.record_loop_iteration().unwrap();

    // Generate some measurement data to have lag values
    monitor.simulate_measurement().unwrap();

    let stats = monitor.get_stats().unwrap();
    let stats_period = stats.get(&TimePeriod::TenSeconds).unwrap();
    assert_eq!(stats_period.loop_metrics.iterations, 1);
    assert!(stats_period.loop_metrics.avg_lag > 0.0);
  ...
  fn test_handle_type_conversion() {
    assert_eq!(HandleType::from_str("timer"), HandleType::Timer);
    assert_eq!(HandleType::from_str("tcp"), HandleType::Tcp);
    assert_eq!(HandleType::from_str("unknown"), HandleType::Unknown);
