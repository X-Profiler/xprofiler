...
static CONFIG_STORE: Lazy<Arc<RwLock<ConfigStore>>> = Lazy::new(|| {
    Arc::new(RwLock::new(ConfigStore::new()))
});

/// Initialize configuration with default values
pub fn initialize_config() -> Result<(), Box<dyn std::error::Error>> {
    let mut store = CONFIG_STORE.write().map_err(|e| format!("Failed to acquire write lock: {}", e))?;
...
pub fn get_config(key: &str) -> Result<Option<ConfigValue>, Box<dyn std::error::Error>> {
    let store = CONFIG_STORE.read().map_err(|e| format!("Failed to acquire read lock: {}", e))?;
    Ok(store.get(key))
}
