...
impl EnvironmentRegistry {
  ...
  pub fn get_statistics(&self) -> RegistryStatistics {
    ...
    let total_gc_count: u32 = self
      .thread_data
      .values()
      .map(|env_data| {
        env_data.gc_statistics.gc_count_full + env_data.gc_statistics.gc_count_incremental
      })
      .sum();

    let total_uv_handles: u32 = self
      .thread_data
      .values()
