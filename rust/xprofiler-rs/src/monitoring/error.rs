...
impl fmt::Display for MonitoringError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MonitoringError::UnsupportedPlatform { operation, platform } => {
                write!(f, "Operation '{}' not supported on platform '{}'", operation, platform)
            }
            MonitoringError::InvalidConfig { parameter, value, reason } => {
                write!(f, "Invalid configuration for '{}' = '{}': {}", parameter, value, reason)
            }
            MonitoringError::ResourceUnavailable { resource, reason } => {
                write!(f, "Resource '{}' unavailable: {}", resource, reason)
            }
            MonitoringError::ParseError { data_type, input, source } => {
                write!(f, "Failed to parse {} from '{}': {}", data_type, input, source)
            }
            MonitoringError::NotInitialized { module } => {
                write!(f, "Monitoring module '{}' not initialized", module)
            }
            MonitoringError::Timeout { operation, duration_ms } => {
                write!(f, "Operation '{}' timed out after {}ms", operation, duration_ms)
            }
            MonitoringError::OutOfMemory { requested_bytes } => {
                write!(f, "Out of memory: failed to allocate {} bytes", requested_bytes)
            }
