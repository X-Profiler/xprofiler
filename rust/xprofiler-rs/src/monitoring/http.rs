...
impl UrlPatternMatcher {
  pub fn new() -> Self {
    ...
    patterns.push((
      Regex::new(r"/api/v\d+/users/\d+").unwrap(),
      "/api/v*/users/*".to_string(),
    ));
    patterns.push((
      Regex::new(r"/api/v\d+/posts/\d+").unwrap(),
      "/api/v*/posts/*".to_string(),
    ));
    patterns.push((
      Regex::new(r"/api/v\d+/orders/\d+").unwrap(),
      "/api/v*/orders/*".to_string(),
    ));
    ...
    patterns.push((
      Regex::new(r"/files/[a-f0-9-]+").unwrap(),
      "/files/*".to_string(),
    ));
...
impl HttpMonitor {
  ...
  fn calculate_stats_for_period(&self, period: TimePeriod) -> MonitoringResult<HttpStats> {
    ...
    // Use end_time (response time) if available, otherwise use start_time
    let relevant_transactions: Vec<_> = completed
      .iter()
      .filter(|t| {
        let transaction_time = t.end_time.unwrap_or(t.start_time);
        transaction_time >= cutoff_time
      })
      .collect();
    ...
    for transaction in &relevant_transactions {
      ...
      *methods
        .entry(transaction.request.method.clone())
        .or_insert(0) += 1;
      ...
      *url_patterns.entry(pattern).or_insert(0) += 1;
      ...
      if let Some(ref response) = transaction.response {
        ...
        *status_codes.entry(response.status_code).or_insert(0) += 1;
        ...
        // Record response time (convert to milliseconds with sub-millisecond precision)
        let response_time_ms = response.response_time.as_secs_f64() * 1000.0;
        response_times.push(response_time_ms);

        // Count slow requests
        let response_time_ms_u64 = response_time_ms as u64;
        if response_time_ms_u64 >= self.slow_request_threshold {
          slow_requests += 1;
        }
        if response_time_ms_u64 >= self.very_slow_request_threshold {
  ...
  fn cleanup(&mut self) -> MonitoringResult<()> {
    ...
    if let Ok(mut active) = self.active_transactions.lock() {
      active.retain(|_, transaction| now.duration_since(transaction.start_time) < timeout);
    }
...
mod tests {
  ...
  fn test_record_request_response() {
    ...
    assert!(monitor
      .record_request("test-123".to_string(), request)
      .is_ok());
    ...
    assert!(monitor
      .record_response("test-123".to_string(), response)
      .is_ok());
  }
  ...
  fn test_get_stats() {
    ...
    for i in 0..5 {
      ...
      monitor
        .record_request(format!("test-{}", i), request)
        .unwrap();
      ...
      monitor
        .record_response(format!("test-{}", i), response)
        .unwrap();
    }
  ...
  fn test_slow_request_tracking() {
    ...
    monitor
      .record_request("slow-1".to_string(), request)
      .unwrap();
    ...
    monitor
      .record_response("slow-1".to_string(), response)
      .unwrap();
  ...
  fn test_url_pattern_matching() {
    ...
    assert_eq!(
      matcher.match_pattern("/api/v1/users/123"),
      "/api/v*/users/*"
    );
