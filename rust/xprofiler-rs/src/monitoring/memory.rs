...
impl MemoryMonitor {
  ...
  fn get_process_memory(&self) -> MonitoringResult<ProcessMemory> {
    ...
    let output_str = String::from_utf8(output.stdout).with_parse_context("ps output", "stdout")?;
...
pub static MEMORY_MONITOR: Lazy<Arc<Mutex<MemoryMonitor>>> =
  Lazy::new(|| Arc::new(Mutex::new(MemoryMonitor::new())));
