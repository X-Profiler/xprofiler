...
impl GcMonitor {
  ...
  fn add_to_frequency_history(&mut self, timestamp: u64) {
    ...
    Self::add_to_history_static(
      &mut self.history_frequency_15s,
      timestamp,
      current_count,
      15,
    );
    Self::add_to_history_static(
      &mut self.history_frequency_30s,
      timestamp,
      current_count,
      30,
    );
    ...
    Self::add_to_history_static(
      &mut self.history_frequency_3m,
      timestamp,
      current_count,
      180,
    );
    Self::add_to_history_static(
      &mut self.history_frequency_5m,
      timestamp,
      current_count,
      300,
    );
  ...
  fn add_to_history_static<T: Copy>(
  ...
  /// Calculate average pause time from history with SIMD optimization
  fn calculate_pause_average(&self, queue: &VecDeque<(u64, f64)>) -> f64 {
    if queue.is_empty() {
      return 0.0;
    }

    let values: Vec<f64> = queue.iter().map(|(_, pause)| *pause).collect();
    self.calculate_average_simd(&values)
  }

  /// SIMD-optimized average calculation for pause times
  ...
  pub fn format_gc_stats(&self) -> String {
    ...
    let incremental_count = stats
      .gc_counts
      .get(&GcType::IncrementalMarking)
      .unwrap_or(&0);

    let scavenge_time = stats
      .gc_durations
      .get(&GcType::Scavenge)
      .unwrap_or(&Duration::ZERO)
      .as_millis();
    let mark_sweep_time = stats
      .gc_durations
      .get(&GcType::MarkSweepCompact)
      .unwrap_or(&Duration::ZERO)
      .as_millis();
    let incremental_time = stats
      .gc_durations
      .get(&GcType::IncrementalMarking)
      .unwrap_or(&Duration::ZERO)
      .as_millis();
...
pub static GC_MONITOR: Lazy<Arc<Mutex<GcMonitor>>> =
  Lazy::new(|| Arc::new(Mutex::new(GcMonitor::new())));
...
pub fn start_gc_monitoring() -> MonitoringResult<()> {
  let mut monitor = GC_MONITOR.lock().map_err(|_| MonitoringError::LockFailed {
