...
use crate::monitoring::memory::{self, HeapStats, MemoryUsage};
...
pub fn init_memory_monitor() -> Result<()> {
  memory::init_memory_monitor().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to initialize memory monitor: {}", e),
    )
  })
}
...
pub fn start_memory_monitoring() -> Result<()> {
  memory::start_memory_monitoring().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to start memory monitoring: {}", e),
    )
  })
}
...
pub fn stop_memory_monitoring() -> Result<()> {
  memory::stop_memory_monitoring().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to stop memory monitoring: {}", e),
    )
  })
}
...
pub fn update_memory_usage() -> Result<()> {
  let mut monitor = memory::MEMORY_MONITOR.lock().map_err(|_| {
    Error::new(
      Status::GenericFailure,
      "Failed to lock memory monitor".to_string(),
    )
  })?;
  monitor.update().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to update memory usage: {}", e),
    )
  })?;
  Ok(())
}
...
pub fn reset_memory_monitor() -> Result<()> {
  memory::reset_memory_monitor().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to reset memory monitor: {}", e),
    )
  })
}
...
pub fn get_memory_usage_history() -> HashMap<String, Option<JsMemoryUsage>> {
  ...
  if let Some(usage) =
    memory::get_memory_usage_for_period(crate::monitoring::TimePeriod::FiveMinutes)
  {
  ...
  if let Some(usage) =
    memory::get_memory_usage_for_period(crate::monitoring::TimePeriod::TenSeconds)
  {
  ...
  if let Some(usage) =
    memory::get_memory_usage_for_period(crate::monitoring::TimePeriod::ThirtySeconds)
  {
  ...
  if let Some(usage) =
    memory::get_memory_usage_for_period(crate::monitoring::TimePeriod::TwoMinutes)
  {
...
pub fn get_memory_usage_mb() -> Option<HashMap<String, f64>> {
  memory::get_memory_usage().map(|usage| {
    ...
    result.insert(
      "heap_used".to_string(),
      usage.heap_used as f64 / 1024.0 / 1024.0,
    );
    result.insert(
      "heap_total".to_string(),
      usage.heap_total as f64 / 1024.0 / 1024.0,
    );
    result.insert(
      "external".to_string(),
      usage.external as f64 / 1024.0 / 1024.0,
    );
    result
  })
}
...
pub fn get_heap_stats_mb() -> Option<HashMap<String, f64>> {
  memory::get_heap_stats().map(|stats| {
    let mut result = HashMap::new();
    result.insert(
      "total_heap_size".to_string(),
      stats.total_heap_size as f64 / 1024.0 / 1024.0,
    );
    result.insert(
      "used_heap_size".to_string(),
      stats.used_heap_size as f64 / 1024.0 / 1024.0,
    );
    result.insert(
      "heap_size_limit".to_string(),
      stats.heap_size_limit as f64 / 1024.0 / 1024.0,
    );
    result.insert(
      "external_memory".to_string(),
      stats.external_memory as f64 / 1024.0 / 1024.0,
    );
    result
  })
}
