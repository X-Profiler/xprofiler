...
use crate::monitoring::gc::{self, GcEvent, GcStats, GcType};
...
impl From<GcStats> for JsGcStats {
  fn from(stats: GcStats) -> Self {
    ...
    for (gc_type, duration) in stats.gc_durations {
      gc_durations.insert(
        gc_type.as_str().to_string(),
        duration.as_secs_f64() * 1000.0,
      );
    }

    for (gc_type, duration) in stats.gc_avg_durations {
      gc_avg_durations.insert(
        gc_type.as_str().to_string(),
        duration.as_secs_f64() * 1000.0,
      );
    }

    let recent_events = stats
      .recent_events
      .into_iter()
...
/// Record a GC event
#[napi]
pub fn record_gc_event(gc_type_str: String, duration_ms: f64, heap_before: f64, heap_after: f64) {
...
pub fn get_gc_summary() -> HashMap<String, f64> {
  ...
  if let Some(stats) = gc::get_gc_stats() {
    ...
    summary.insert(
      ...
      stats.total_gc_time.as_secs_f64() * 1000.0,
    );
    ...
    for (gc_type, duration) in stats.gc_durations {
      summary.insert(
        format!("{}_time_ms", gc_type.as_str()),
        duration.as_secs_f64() * 1000.0,
      );
    }
...
pub fn update_gc_stats() -> Result<()> {
  let _monitor = gc::GC_MONITOR.lock().map_err(|_| {
    Error::new(
      Status::GenericFailure,
      "Failed to lock GC monitor".to_string(),
    )
  })?;
