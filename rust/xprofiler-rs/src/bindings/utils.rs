...
use crate::utils::{
    get_process_info, get_system_info, format_bytes, format_duration,
    get_node_version, get_v8_version, get_platform_info
};
...
pub fn get_process_info_js() -> Result<JsProcessInfo> {
    let info = get_process_info()
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get process info: {}", e)))?;
...
pub fn get_system_info_js() -> Result<JsSystemInfo> {
    let info = get_system_info()
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get system info: {}", e)))?;
...
pub fn get_platform_info_js() -> Result<JsPlatformInfo> {
    let info = get_platform_info()
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get platform info: {}", e)))?;
...
pub fn get_executable_path() -> Result<String> {
    std::env::current_exe()
        .map(|path| path.to_string_lossy().to_string())
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get executable path: {}", e)))
}
...
pub fn generate_uuid() -> String {
    ...
    let _ = write!(uuid, "{:08x}-{:04x}-4{:03x}-{:04x}-{:012x}", 
        (timestamp >> 96) as u32,
