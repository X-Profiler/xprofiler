...
pub fn init_cpu_monitor() -> Result<()> {
  cpu::init_cpu_monitor().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to initialize CPU monitor: {}", e),
    )
  })
}
...
pub fn start_cpu_monitoring() -> Result<()> {
  cpu::start_cpu_monitor().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to start CPU monitoring: {}", e),
    )
  })
}
...
pub fn stop_cpu_monitoring() -> Result<()> {
  cpu::stop_cpu_monitor().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to stop CPU monitoring: {}", e),
    )
  })
}
...
pub fn get_current_cpu_usage() -> Result<JsCpuUsage> {
  let monitor = cpu::CPU_MONITOR.lock().map_err(|_| {
    Error::new(
      Status::GenericFailure,
      "Failed to lock CPU monitor".to_string(),
    )
  })?;
...
pub fn update_cpu_usage() -> Result<()> {
  let mut monitor = cpu::CPU_MONITOR.lock().map_err(|_| {
    Error::new(
      Status::GenericFailure,
      "Failed to lock CPU monitor".to_string(),
    )
  ...
  monitor.update().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to update CPU usage: {}", e),
    )
  })?;
  Ok(())
}
...
pub fn reset_cpu_monitor() -> Result<()> {
  cpu::reset_cpu_monitor().map_err(|e| {
    Error::new(
      Status::GenericFailure,
      format!("Failed to reset CPU monitor: {}", e),
    )
  })
}
