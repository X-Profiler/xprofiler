...
use crate::monitoring::libuv::{HandleType, LibuvMonitor, LibuvStats};
...
fn get_libuv_monitor() -> Arc<Mutex<LibuvMonitor>> {
  LIBUV_MONITOR
    .get_or_init(|| Arc::new(Mutex::new(LibuvMonitor::new())))
    .clone()
}
...
impl From<LibuvStats> for JsLibuvStats {
  fn from(stats: LibuvStats) -> Self {
    let handle_counts = stats
      .handle_counts
      .into_iter()
      .map(|(k, v)| (k.as_str().to_string(), v))
      .collect();

    let active_handle_counts = stats
  ...
  if let Ok(mut monitor) = monitor.lock() {
    monitor.start().map_err(|e| {
      Error::new(
        ...
        format!("Failed to start libuv monitoring: {}", e),
      )
    })
  } else {
    Err(Error::new(
      Status::GenericFailure,
      "Failed to lock libuv monitor".to_string(),
    ))
  }
}
...
pub fn stop_libuv_monitoring_js() -> Result<()> {
  ...
  if let Ok(mut monitor) = monitor.lock() {
    monitor.stop().map_err(|e| {
      Error::new(
        Status::GenericFailure,
        format!("Failed to stop libuv monitoring: {}", e),
      )
    })
  } else {
    Err(Error::new(
...
pub fn update_libuv_handle_status_js(handle_id: String, is_active: bool) {
  ...
  if let Ok(monitor) = monitor.lock() {
    ...
    let _ = monitor.record_measurement(
      HandleType::Other,
...
pub fn format_libuv_stats_js() -> String {
  ...
  if let Ok(monitor) = monitor.lock() {
    if let Ok(stats) = monitor.get_stats_for_period(TimePeriod::TenSeconds) {
      return format!(
        "Libuv Stats: {} handles, avg measurement: {:.2}",
