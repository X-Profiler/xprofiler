...
impl StackFrame {
    ...
    pub fn with_location(mut self, file_name: String, line_number: u32, column_number: Option<u32>) -> Self {
    ...
    pub fn get_stats(&self) -> XProfilerResult<CpuProfileStats> {
        ...
        for sample in samples.iter() {
            for frame in &sample.call_stack.frames {
                *function_frequency.entry(frame.function_name.clone()).or_insert(0) += 1;
            }
        }
        
        // Get hot functions (top 10)
        let mut hot_functions: Vec<(String, usize)> = function_frequency.iter()
...
impl Profiler for CpuProfiler {
    ...
    fn get_results(&self) -> XProfilerResult<String> {
        ...
        serde_json::to_string_pretty(&stats)
            .map_err(|e| XProfilerError::Monitoring {
