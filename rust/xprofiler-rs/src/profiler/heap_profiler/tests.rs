...
mod tests {
    ...
    fn test_record_allocation() {
        ...
        profiler.record_allocation(address, size, alloc_type.clone())
    ...
    fn test_record_deallocation() {
        ...
        // First allocate
        profiler.record_allocation(address, 1024, AllocationType::Malloc)
            .expect("Failed to record allocation");
    ...
    fn test_detect_memory_leaks() {
        ...
        profiler.record_allocation(0x2000, 2048, AllocationType::New)
            .expect("Failed to record allocation");
        profiler.record_allocation(0x3000, 512, AllocationType::Calloc)
            .expect("Failed to record allocation");
    ...
    fn test_get_heap_stats() {
        ...
        let stats = profiler.get_stats()
            .expect("Failed to get heap stats");
    ...
    fn test_heap_profiler_get_results() {
        ...
        let _: serde_json::Value = serde_json::from_str(&results)
            .expect("Results should be valid JSON");
    }
    ...
    fn test_allocation_event_serialization() {
        ...
        let deserialized: AllocationEvent = serde_json::from_str(&json)
    ...
    fn test_deallocation_event_serialization() {
        ...
        let json = serde_json::to_string(&event).expect("Failed to serialize event");
        let deserialized: DeallocationEvent = serde_json::from_str(&json)
            .expect("Failed to deserialize event");
    ...
    fn test_heap_profile_stats_serialization() {
        ...
        let deserialized: HeapProfileStats = serde_json::from_str(&json)
        ...
        assert_eq!(stats.total_bytes_allocated, deserialized.total_bytes_allocated);
    }
...
mod integration_tests {
    ...
    fn test_heap_profiler_realistic_scenario() {
        let mut profiler = HeapProfiler::new(ProfilerConfig::default())
        ...
        for i in 0..100 {
            ...
            profiler.record_allocation(
                address,
                size as usize,
                if i % 3 == 0 { AllocationType::Malloc } else { AllocationType::New }
            ).expect("Failed to record allocation");
        }
        
        // Deallocate some memory (not all - simulate leaks)
        for i in (0..80).step_by(2) {
            profiler.record_deallocation(addresses[i])
                .expect("Failed to record deallocation");
        }
        ...
        let leaks = profiler.detect_memory_leaks().expect("Failed to detect leaks");
        assert!(leaks.len() > 0);
    ...
    fn test_heap_profiler_concurrent_access() {
        let profiler = Arc::new(Mutex::new(
            HeapProfiler::new(ProfilerConfig::default())
                .expect("Failed to create profiler")
        ));
    ...
    fn test_heap_profiler_stress_test() {
        ...
        for i in 0..1000 {
            ...
            profiler.record_allocation(address, size as usize, AllocationType::Malloc)
                .expect("Failed to record allocation");
            ...
            if i > 100 && i % 10 == 0 {
                ...
                profiler.record_deallocation(dealloc_address)
                    .expect("Failed to record deallocation");
            }
        }
