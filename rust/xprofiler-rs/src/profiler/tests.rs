...
use super::gc_profiler::{GcEventType, GcPhase, GcProfiler};
use super::heap_profiler::{AllocationType, HeapProfiler};
...
mod integration_tests {
  ...
  fn test_all_profilers_creation() {
    ...
    let cpu_profiler = CpuProfiler::new(config.clone()).expect("Failed to create CPU profiler");
    let heap_profiler = HeapProfiler::new(config.clone()).expect("Failed to create Heap profiler");
    let gc_profiler = GcProfiler::new(config.clone()).expect("Failed to create GC profiler");
  ...
  fn test_profiler_memory_usage() {
    ...
    let mut cpu_profiler = CpuProfiler::new(config.clone()).expect("Failed to create CPU profiler");
    let mut heap_profiler =
      HeapProfiler::new(config.clone()).expect("Failed to create Heap profiler");
    let mut gc_profiler = GcProfiler::new(config.clone()).expect("Failed to create GC profiler");

    // Start profilers
    cpu_profiler.start().expect("Failed to start CPU profiler");
    heap_profiler
      .start()
      .expect("Failed to start Heap profiler");
    ...
    let cpu_results = cpu_profiler
      .get_results()
      .expect("Failed to get CPU results"); // CPU profiler returns JSON string
    let heap_results = heap_profiler
      .get_results()
      .expect("Failed to get heap results"); // Heap profiler returns JSON string
  ...
  fn test_profiler_reset_functionality() {
    ...
    // Verify data exists
    let cpu_results_before = cpu_profiler
      .get_results()
      .expect("Failed to get CPU results before reset"); // CPU profiler returns JSON string
    let heap_results_before = heap_profiler
      .get_results()
      .expect("Failed to get heap results before reset"); // Heap profiler returns JSON string
    let gc_results_before = gc_profiler
      .get_results()
      .expect("Failed to get GC results before reset"); // GC profiler returns JSON string
    ...
    heap_profiler
      .reset()
      .expect("Failed to reset Heap profiler");
    ...
    let cpu_results_after = cpu_profiler
      .get_results()
      .expect("Failed to get CPU results after reset"); // CPU profiler returns JSON string
    let heap_results_after = heap_profiler
      .get_results()
      .expect("Failed to get heap results after reset"); // Heap profiler returns JSON string
    let gc_results_after = gc_profiler
      .get_results()
      .expect("Failed to get GC results after reset"); // GC profiler returns JSON string
