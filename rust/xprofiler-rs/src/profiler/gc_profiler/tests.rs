...
mod tests {
    ...
    fn test_calculate_gc_stats() {
        ...
        assert_eq!(stats.total_memory_reclaimed, (1024 - 512) + (2048 - 1024) + (4096 - 2048) + (8192 - 4096));
    ...
    fn test_gc_event_serialization() {
        ...
        let deserialized: GcEvent = serde_json::from_str(&json)
    ...
    fn test_gc_profile_stats_serialization() {
        ...
        generation_stats.insert("young".to_string(), GcGenerationStats {
        ...
        let deserialized: GcProfileStats = serde_json::from_str(&json)
...
mod integration_tests {
    ...
    fn test_gc_profiler_realistic_scenario() {
        let mut profiler = GcProfiler::new(ProfilerConfig::default())
    ...
    fn test_gc_profiler_concurrent_access() {
        let profiler = Arc::new(Mutex::new(
            GcProfiler::new(ProfilerConfig::default())
                .expect("Failed to create profiler")
        ));
            ...
            thread::spawn(move || {
                for i in 0..10 {
                    ...
                    
                    {
                        ...
                        p.record_gc_event(
                            if i % 3 == 0 { GcEventType::MinorGc } else { GcEventType::MajorGc },
                            GcPhase::Start,
                            1000 + (i * 100),
                            (1024 * (i + 1)) as usize,
                            (512 * (i + 1)) as usize,
    ...
    fn test_gc_profiler_memory_efficiency() {
        ...
        assert!(stats.gc_type_stats.len() > 0, "Should have GC type statistics");
